 ...
 */
function semanticIdentity(transaction: Transaction): string {
...
export function migrateLegacyEditTargets(
    ...
    const rebuiltIds = new Set(
        [...rebuilt.allParentChildTransactions].map((transaction) =>
            transaction.id,
        ),
    );
    ...
    for (const transaction of rebuilt.allParentChildTransactions) {
        addToIndex(
            rebuiltBySemanticIdentity,
            semanticIdentity(transaction),
            transaction,
        );
    }
    ...
    for (const edit of edits) {
        ...
        const scopeFilters = cloned.scopeFilters.map((scopeFilter) => {
            ...
            const migratedParameters = scopeFilter.parameters.map((targetId) => {
                ...
                const candidates = previousTransaction == null
                    ? []
                    : rebuiltBySemanticIdentity.get(
                        semanticIdentity(previousTransaction),
                    ) ?? [];
