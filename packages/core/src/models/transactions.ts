...
function normalizeAccountInfo(value: unknown): AccountInfo {
    ...
    return {
        ...
        interAccountNameTags: Array.isArray(
            raw.interAccountNameTags ?? raw.InterAccountNameTags,
        )
            ? (raw.interAccountNameTags
                ?? raw.InterAccountNameTags) as string[]
            : null,
    };
}
    ...
    static filterTransaction(scopeFilter: ScopeFilter, transaction: Transaction): boolean {
        switch (scopeFilter.type) {
            ...
            case ScopeType.AmountRange: {
                ...
                if (isNegative) {
                    return transaction.correctedAmount <= minVal * -1
                        && transaction.correctedAmount >= maxVal * -1;
                } else {
                    return transaction.correctedAmount >= minVal
                        && transaction.correctedAmount <= maxVal;
                }
            }
            default:
