...
function getContentFields(
    ...
    return [
        ...
        transactionReasonInfo.find((item) => item.value === transactionReason)
            ?.key ?? transactionReason.toString(),
        amount.toString(),
...
export class Transaction {
    ...
    static fromData(data: TransactionData): Transaction {
        const providerAttributes = normalizeDictionary<string>(
            data.providerAttributes,
        );
        const rawChildren = normalizeDictionary<TransactionData>(data.children);
        const children: Record<string, TransactionData> | null = rawChildren
            == null
            ? null
            : Object.fromEntries(
    ...
    get correctedAmount(): number {
        ...
        return edited != null && !edited.isVoided
            ? edited.value
            : this.data.amount;
    }
