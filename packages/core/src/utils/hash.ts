 ...
 *
 * Uses the built-in {@link crypto.randomUUID} when available. Browsers expose
 * that convenience method only in secure contexts, so deployments served over
 * plain HTTP fall back to the equally secure {@link crypto.getRandomValues}
 * primitive and set the RFC 4122 version/variant bits locally.
 *
