...
function makeTransaction(accountId: string, overrides?: Partial<ImportedValues>): Transaction {
  return Transaction.create('import-001', accountId, false, {
    amount: -100.0,
...
describe('isMissingAmountTolerable', () => {
  it('should return true when missing amount is less than 2% of parent amount', () => {
    const parent = makeTransaction('acct-chase', { amount: -100.0 });
    // 2% of 100 = 2.00; missing = 1.50 < 2.00
    expect(isMissingAmountTolerable(parent, 1.5)).toBe(true);
  });

  it('should return true when missing amount is less than $0.50', () => {
    const parent = makeTransaction('acct-chase', { amount: -10.0 });
    // 2% of 10 = 0.20; missing = 0.30 > 0.20, but 0.30 < 0.50
    expect(isMissingAmountTolerable(parent, 0.3)).toBe(true);
  });

  it('should return false when missing amount exceeds both thresholds', () => {
    const parent = makeTransaction('acct-chase', { amount: -50.0 });
    // 2% of 50 = 1.00; missing = 5.00 > 1.00 and 5.00 > 0.50
    expect(isMissingAmountTolerable(parent, 5.0)).toBe(false);
  });
  ...
  it('should return false for large missing amounts relative to parent', () => {
    const parent = makeTransaction('acct-chase', { amount: -20.0 });
    // 2% of 20 = 0.40; missing = 3.00 > 0.40 and 3.00 > 0.50
    expect(isMissingAmountTolerable(parent, 3.0)).toBe(false);
  });
...
describe('GenericTxParentChildMatcher.handleIncompleteParent', () => {
  ...
  it('should create adjustment child for tolerable positive missing amount', () => {
    ...
    const result = matcher.handleIncompleteParent(parent, txns, 0.3);
    ...
    expect(childData.amount).toBe(-0.3);
  ...
  it('should create MatchAdjustmentCredit for negative missing amount', () => {
    ...
    const result = matcher.handleIncompleteParent(parent, txns, -0.2);
    expect(result).toBe(true);
    ...
    const childData = children![childIds[0]!]!;
    expect(childData.amount).toBe(0.2);
  ...
  it('should return false for intolerable missing amount', () => {
    ...
    const result = matcher.handleIncompleteParent(parent, txns, 10.0);
  ...
  it('should register the import info in the collection', () => {
    ...
    matcher.handleIncompleteParent(parent, txns, 0.1);
