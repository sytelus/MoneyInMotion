...
function makeTransaction(
  ...
  return Transaction.create(importId, accountId, false, {
    amount: -25.99,
...
describe('GenericOrderMatcher.handleIncompleteParent', () => {
  it('should return true when missingChildAmount is zero', () => {
    ...
    const matcher = new GenericOrderMatcher(
      orderAcct,
      'shipping charge',
      'tax charged',
      'total promotions',
  ...
  it('should create adjustment children for discount, shipping, tax, and remainder', () => {
    ...
    const missingAmount = 2.0 + 5.99 + 8.25; // = 16.24
  ...
  it('does not create a phantom adjustment for a sub-cent floating-point remainder', () => {
    const orderAcct = makeOrderAccount();
    const ccAcct = makeCreditCardAccount();
    const imp = makeImportInfo();
    const txns = new Transactions('test');
    ...
    expect(matcher.handleIncompleteParent(parent, txns, -5.98)).toBe(true);

    const children = Object.values(parent.children ?? {});
    expect(children).toHaveLength(1);
    expect(children[0]?.entityName).toMatch(/^Shipping/);
    expect(children[0]?.amount).toBe(-5.98);
  });
  ...
  it('should return false when remaining missing amount is intolerable', () => {
    ...
    const result = matcher.handleIncompleteParent(parent, txns, 50.0);
