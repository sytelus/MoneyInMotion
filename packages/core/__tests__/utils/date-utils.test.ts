...
describe('parseDate', () => {
  ...
  it('should reject impossible ISO date-only values', () => {
    expect(() => parseDate('2024-02-30')).toThrow('Invalid date string');
    expect(() => parseDate('2023-02-29')).toThrow('Invalid date string');
  });

  it('should accept an ISO leap day', () => {
    expect(parseDate('2024-02-29').toISOString()).toBe('2024-02-29T00:00:00.000Z');
  });

  it('should reject impossible ISO timestamp calendar dates', () => {
    expect(() => parseDate('2024-02-30T12:00:00Z')).toThrow('Invalid date string');
    expect(parseDate('2024-02-29T12:00:00Z').toISOString()).toBe('2024-02-29T12:00:00.000Z');
  });

  it('should preserve literal ISO years below 100', () => {
    expect(parseDate('0099-01-02').toISOString()).toBe('0099-01-02T00:00:00.000Z');
  });
