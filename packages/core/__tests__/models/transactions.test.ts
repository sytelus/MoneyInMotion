...
describe('Transactions construction and addNew', () => {
    ...
    it('serializes the live child graph after child edits', () => {
        ...
        txns.apply(makeEdit({
            scopeFilters: [
                createScopeFilter(ScopeType.TransactionId, [child.id]),
            ],
            values: { note: editValue('Persist this child correction') },
        }));

        const serialized = JSON.parse(
            JSON.stringify(txns.serialize()),
        ) as TransactionsData;
        ...
        expect(restored.getTransaction(child.id)?.note).toBe(
            'Persist this child correction',
        );
    });
...
describe('Transactions.fromData', () => {
    ...
    it('should normalize nested C# dictionaries and embedded edit aggregates', () => {
        ...
        parent.children = [{ Key: child.id, Value: {
            ...child,
        ...
        expect(restored.getTransaction(child.id)?.providerAttributes)
            .toEqual({ 'legacy field': 'legacy value' });
    });
