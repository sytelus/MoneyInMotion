...
describe('Transactions construction and addNew', () => {
  ...
  it('serializes the live child graph after child edits', () => {
    const txns = new Transactions('test');
    const acct = makeAccountInfo();
    const imp = makeImportInfo();
    const parent = makeTransaction({
      amount: -50,
      entityName: 'PARENT CHARGE',
    });
    ...
    txns.addNew(child, acct, imp, false);
    txns.relateParentChild(parent.id, child.id);

    txns.apply(
      makeEdit({
        scopeFilters: [createScopeFilter(ScopeType.TransactionId, [child.id])],
        values: { note: editValue('Persist this child correction') },
      }),
    );

    const serialized = JSON.parse(JSON.stringify(txns.serialize())) as TransactionsData;
    const restored = Transactions.fromData(serialized);

    expect(restored.topLevelTransactionCount).toBe(1);
    expect(restored.allTransactionCount).toBe(2);
    expect(restored.getTransaction(child.id)?.note).toBe('Persist this child correction');
  });
...
describe('Transactions.filterTransaction', () => {
  ...
  it('ScopeType.TransactionReason should match numeric reason', () => {
    ...
    const noMatch = createScopeFilter(ScopeType.TransactionReason, [String(TransactionReason.Fee)]);
    expect(Transactions.filterTransaction(noMatch, tx)).toBe(false);
  });
...
describe('Transactions.apply', () => {
  ...
  it('validates exact IDs independently from other ANDed scope filters', () => {
    const { txns, tx1 } = makePopulatedTransactions();
    const edit = makeEdit({
      scopeFilters: [
        createScopeFilter(ScopeType.TransactionId, [tx1.id]),
        createScopeFilter(ScopeType.EntityName, ['does-not-match']),
      ],
      values: { note: editValue('No matching transaction') },
    });

    expect(txns.apply(edit, false)).toEqual([]);
    expect(txns.editsCount).toBe(1);
    expect(txns.getTransaction(tx1.id)?.note).toBeNull();
  });

  it('rejects a missing exact ID even when the edit has additional filters', () => {
    const { txns } = makePopulatedTransactions();
    const edit = makeEdit({
      scopeFilters: [
        createScopeFilter(ScopeType.TransactionId, ['missing']),
        createScopeFilter(ScopeType.EntityName, ['anything']),
      ],
      values: { note: editValue('Must be rejected') },
    });
...
describe('Transactions.fromData', () => {
  ...
  it('should normalize nested C# dictionaries and embedded edit aggregates', () => {
    const parent = makeTransaction({
      entityName: 'Parent',
      amount: -10,
    }).toData();
    ...
    parent.children = [
      {
        ...
        Value: {
          ...child,
          providerAttributes: [{ Key: 'legacy field', Value: 'legacy value' }],
        },
      },
    ] as unknown as typeof parent.children;
    const legacyEdit = makeEdit({
      id: 'legacy-edit',
      scopeFilters: [createScopeFilter(ScopeType.All, [])],
      values: { note: editValue('Loaded from legacy') },
    });
    const legacyData = {
      name: 'LatestMerged.json',
      topItems: [{ Key: parent.id, Value: parent }],
      accountInfos: [
        {
          Key: 'acct-amex',
          Value: {
      ...
      edits: { sourceId: 'LatestMerged.json', edits: [legacyEdit] },
    } as unknown as TransactionsData;
    ...
    expect(restored.getAccountInfo('acct-amex')).toMatchObject({
      id: 'acct-amex',
      instituteName: 'AmericanExpress',
      requiresParent: false,
    });
    expect(restored.getImportInfo('import-001')).toMatchObject({
      id: 'import-001',
      portableAddress: 'statement.CSV',
      format: '.CSV',
    });
  ...
  it('should round-trip serialize and deserialize', () => {
    ...
    expect([...restored.topLevelTransactions]).toHaveLength([...txns.topLevelTransactions].length);
...
describe('Transactions.applyEdits', () => {
  ...
  it('rejects a batch atomically when a later exact-ID target is missing', () => {
    const { txns, tx1 } = makePopulatedTransactions();
    ...
    edits.add(
      makeEdit({
        ...
        values: { note: editValue('invalid') },
      }),
    );

    expect(() => txns.applyEdits(edits, false)).toThrow(/only 0 were found/);
    expect(txns.getTransaction(tx1.id)?.note).toBeNull();
    expect(txns.editsCount).toBe(0);
  });

  it('rejects a conflicting duplicate atomically before applying new edits', () => {
    const { txns, tx1, tx2 } = makePopulatedTransactions();
    txns.apply(
      makeEdit({
        id: 'existing',
        scopeFilters: [createScopeFilter(ScopeType.TransactionId, [tx1.id])],
        values: { note: editValue('original') },
      }),
    );
    ...
    edits.add(
      makeEdit({
        id: 'new-edit',
        scopeFilters: [createScopeFilter(ScopeType.TransactionId, [tx2.id])],
        values: { note: editValue('must not leak') },
      }),
    );
    edits.add(
      makeEdit({
        id: 'existing',
        scopeFilters: [createScopeFilter(ScopeType.TransactionId, [tx1.id])],
        values: { note: editValue('conflict') },
      }),
    );

    expect(() => txns.applyEdits(edits, false)).toThrow(/Conflicting/);
    expect(txns.getTransaction(tx1.id)?.note).toBe('original');
    expect(txns.getTransaction(tx2.id)?.note).toBeNull();
    expect(txns.editsCount).toBe(1);
  });
  ...
  it('should ignore duplicate edit IDs when replaying the same edit', () => {
    ...
    expect(txns.getTransaction(tx1.id)!.appliedEditIdsDescending).toEqual(['edit-replay']);
  });
});
