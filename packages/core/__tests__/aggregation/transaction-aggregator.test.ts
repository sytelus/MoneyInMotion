...
import { editValue } from '../../src/models/transaction-edit.js';
...
describe('TransactionAggregator', () => {
  it('should accumulate totals from added transactions', () => {
    ...
    agg.add(
      makeTx({ amount: 20, entityName: 'Refund', transactionReason: TransactionReason.Return }),
    );
  ...
  it('should aggregate corrected amount and reason values', () => {
    const transaction = makeTx({ amount: -50 });
    transaction.applyEdit({
      id: 'correct-financial-values',
    ...
    const aggregator = new TransactionAggregator({ name: 'root' });
  ...
  describe('hierarchical grouping', () => {
    it('should delegate transactions to sub-aggregators via subAggregateFn', () => {
      ...
      root.add(
        makeTx({ amount: 20, entityName: 'Income1', transactionReason: TransactionReason.Return }),
      );
  ...
  describe('key counters', () => {
    it('should track transaction reasons', () => {
      ...
      agg.add(
        makeTx({ amount: -10, entityName: 'A', transactionReason: TransactionReason.Purchase }),
      );
      agg.add(makeTx({ amount: -5, entityName: 'B', transactionReason: TransactionReason.Fee }));
      agg.add(
        makeTx({ amount: -15, entityName: 'C', transactionReason: TransactionReason.Purchase }),
      );
