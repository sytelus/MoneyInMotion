...
export function discoverAccountConfigs(
        ...
        try {
            ...
            results.push({
                ...
                relativeDirectory: path
                    .relative(statementsDir, dirPath)
                    .split(path.sep)
                    .join('/'),
            });
        } catch (err) {
            console.warn(
                `Skipping invalid account config "${configPath}": ${err instanceof Error ? err.message : String(err)}`,
            );
        }
    }
    ...
    for (const entry of entries) {
        results.push(
            ...discoverAccountConfigs(statementsDir, path.join(dirPath, entry.name)),
        );
    }
    return results;
}
