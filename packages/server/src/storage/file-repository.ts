    ...
    getStatementLocations(
        ...
        if (accountConfig) {
            for (const fileFilter of accountConfig.fileFilters) {
                ...
                const entries = fs.readdirSync(dirPath).sort((left, right) =>
                ...
                for (const entry of entries) {
                    ...
                    // Legacy data was created on case-insensitive Windows
                    // volumes, so `*.csv` must also match `.CSV` when the web
