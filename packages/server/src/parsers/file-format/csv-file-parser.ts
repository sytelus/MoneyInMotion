...
export class CsvFileParser implements FileFormatParser {
  ...
  parse(content: string, settings?: ParserSettings): ParsedRow[] {
    ...
    for (const columns of rawRows) {
      ...
      // Some historical hand-edited exports contain an unquoted
      // thousands separator in their final amount field (for example
      // `...,--6,408.99`). Reassemble only that unambiguous final-field
      // case. Silently truncating arbitrary extra columns would turn a
      // malformed statement into incorrect financial data.
      if (trimmedColumns.length > headerColumns.length) {
        const finalHeader = headerColumns.at(-1)?.toLowerCase();
        const finalColumnIndex = headerColumns.length - 1;
        const overflow = trimmedColumns.slice(finalColumnIndex);
        const isRecoverableFinalAmount =
          ['amount', 'debit', 'credit'].includes(finalHeader ?? '') &&
          overflow.length === 2 &&
          /^-*[\d$]+$/.test(overflow[0] ?? '') &&
          /^\d{3}(?:\.\d+)?$/.test(overflow[1] ?? '');

        if (isRecoverableFinalAmount) {
          trimmedColumns = [...trimmedColumns.slice(0, finalColumnIndex), overflow.join(',')];
        } else {
          throw new Error(
            `CSV row has ${trimmedColumns.length} fields but the header has ${headerColumns.length}. ` +
   ...
   */
  transformHeaderColumnNames(columns: string[], ignoreColumns?: Set<string>): string[] {
    return columns
      .map((c) => c.toLowerCase().trim())
      .map((c) => (ignoreColumns && ignoreColumns.has(c) ? `_${c}` : c));
  }
}
