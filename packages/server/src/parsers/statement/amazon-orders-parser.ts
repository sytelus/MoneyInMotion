...
export class AmazonOrdersParser extends StatementParserBase {
  ...
  protected override setCalculatedAttributes(importedValues: ImportedValues): void {
    ...
    importedValues.lineItemType =
      'item subtotal' in attrs ? LineItemType.ItemSubtotal : LineItemType.None;

    // Parent-child match filter
    importedValues.parentChildMatchFilter = `${attrs['order id'] ?? ''}|${attrs['carrier name & tracking number'] ?? ''}`;
    ...
    if (isNullOrWhitespace(importedValues.entityName)) {
      if (importedValues.lineItemType !== LineItemType.None) {
        importedValues.entityName = `Amazon ASIN# ${attrs['asin/isbn'] ?? ''} Sold By ${attrs['seller'] ?? ''}, Order# ${attrs['order id'] ?? ''}, Shipment# ${attrs['carrier name & tracking number'] ?? ''}`;
      } else {
        importedValues.entityName = `Amazon Order# ${attrs['order id'] ?? ''}, Shipment# ${attrs['carrier name & tracking number'] ?? ''}`;
      }
  ...
  protected override validateImportedValues(importedValues: ImportedValues): boolean {
    ...
    const shippingStatus = (
      attrs['shipment/order condition'] ??
      attrs['order status'] ??
      ''
    ).toLowerCase();
