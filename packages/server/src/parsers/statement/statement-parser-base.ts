...
export abstract class StatementParserBase {
  ...
  getTransactionImportedValues(): ImportedValues[] {
    ...
    for (const row of this.getTransactionProperties()) {
      ...
      importedValues.transactionReason = this.inferTransactionReason(
        importedValues as ImportedValues,
      );
  ...
  protected inferTransactionReason(importedValues: ImportedValues): number {
    ...
    if (importedValues.transactionReason === UnknownAdjustment) {
      importedValues.transactionReason =
        amount >= 0 ? TransactionReason.IncomeAdjustment : TransactionReason.ExpenseAdjustment;
    }
    ...
    if (
      ...
      if (amount > 0 && entityName != null && entityName.toUpperCase().includes('INTEREST')) {
        ...
        amount > 0 &&
        entityName != null &&
        entityName.toUpperCase().includes('POINTS CREDIT')
      ) {
    ...
    switch (columnType) {
      ...
      case StatementColumnType.EntityName:
        importedValues.entityName =
          this.setImportedValueText(importedValues.entityName, columnValue, columnType) ?? '';
        break;
      ...
      case StatementColumnType.ProviderCategoryName:
        importedValues.providerCategoryName = this.setImportedValueText(
          importedValues.providerCategoryName,
          columnValue,
          columnType,
        );
        break;
  ...
  protected parseAmount(columnValue: string): number {
    ...
    // A small number of legacy manually-normalized exports contain a
    // doubled sign such as `--6,408.99`. Interpret consecutive minus
    // signs mathematically (two negatives are positive) instead of
    // discarding or partially parsing the value.
    const repeatedSign = cleaned.match(/^(-{2,})(\d.*)$/);
    if (repeatedSign) {
      cleaned = repeatedSign[1]!.length % 2 === 0 ? repeatedSign[2]! : `-${repeatedSign[2]!}`;
    }
  ...
  protected parseDateValue(columnValue: string): string {
    try {
      return parseDate(columnValue).toISOString();
    } catch {
  ...
  protected parseTransactionReason(columnValue: string): number {
    ...
    const trimmed = columnValue.trim();
    if (/^[+-]?\d+$/.test(trimmed)) {
