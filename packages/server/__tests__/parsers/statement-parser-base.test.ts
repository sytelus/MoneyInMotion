...
describe('StatementParserBase (via GenericStatementParser)', () => {
  describe('column mapping', () => {
    it('maps standard column names to correct imported value fields', () => {
      const csv = ['Transaction Date,Description,Amount', '01/15/2024,WHOLE FOODS,-45.99'].join(
        '\n',
      );
    ...
    it('interprets a doubled negative sign mathematically', () => {
      const parser = new GenericStatementParser(
        'Type,Date,Description,Amount\nPayment,09/01/2015,Thank you,--6,408.99\n',
        ContentType.Csv,
      );

      expect(parser.getTransactionImportedValues()[0]!.amount).toBe(6408.99);
    });

    it('rejects amounts with trailing non-numeric characters', () => {
      const parser = new GenericStatementParser(
        'Date,Description,Amount\n01/15/2024,Broken,12 dollars\n',
        ContentType.Csv,
      );

      expect(() => parser.getTransactionImportedValues()).toThrow('Cannot parse amount');
    });

    it('rejects impossible calendar dates', () => {
      const parser = new GenericStatementParser(
        'Date,Description,Amount\n02/30/2024,Broken,-12\n',
        ContentType.Csv,
      );

      expect(() => parser.getTransactionImportedValues()).toThrow('Cannot parse date');
    });
    ...
    it('maps debit and credit columns separately', () => {
      ...
      expect(values[0]!.amount).toBe(-50.0); // debit is negated
      expect(values[1]!.amount).toBe(25.0); // credit is positive
    });
    ...
    it('maps check reference column', () => {
      const csv = ['Date,Description,Amount,ChkRef', '01/15/2024,CHECK PAYMENT,-500.00,1234'].join(
        '\n',
      );
  ...
  describe('transaction reason inference', () => {
    ...
    it('rejects partially numeric transaction reasons', () => {
      const parser = new GenericStatementParser(
        'Date,Description,Amount,Type\n01/15/2024,STORE,-25.00,4junk\n',
        ContentType.Csv,
      );

      expect(() => parser.getTransactionImportedValues()).toThrow('Raw TransactionType');
    });
  });
