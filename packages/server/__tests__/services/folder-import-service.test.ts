...
describe('stageAndPromoteFolder', () => {
    ...
    it('stages all files, promotes new content, and records duplicates/rejections', () => {
        ...
        fs.writeFileSync(
            path.join(config.statementsDir, 'Amex', 'existing.csv'),
            existingContent,
        );
        ...
        const result = stageAndPromoteFolder(
            config,
            [
                { originalName: 'new.csv', buffer: newContent, size: newContent.length },
                {
                    originalName: 'same-again.csv',
                    buffer: existingContent,
                    size: existingContent.length,
                },
                { originalName: 'notes.txt', buffer: Buffer.from('no'), size: 2 },
                { originalName: 'orphan.csv', buffer: newContent, size: newContent.length },
            ],
            [
        ...
        expect(
            fs.readFileSync(
                path.join(config.statementsDir, 'Amex', 'new.csv'),
                'utf-8',
            ),
        ).toBe(newContent.toString());
    ...
    it('keeps both files when a filename is reused for different content', () => {
        ...
        fs.writeFileSync(
            path.join(config.statementsDir, 'Amex', 'statement.csv'),
            first,
        );

        const result = stageAndPromoteFolder(
            config,
            [{ originalName: 'statement.csv', buffer: second, size: second.length }],
        ...
        expect(
            fs.readFileSync(
                path.join(config.statementsDir, 'Amex', 'statement (1).csv'),
                'utf-8',
            ),
    ...
    it('rejects traversal paths before writing a staging batch', () => {
        ...
        expect(() =>
            stageAndPromoteFolder(
                config,
                [{ originalName: 'escape.csv', buffer: Buffer.from('x'), size: 1 }],
                ['upload/../escape.csv'],
            ),
