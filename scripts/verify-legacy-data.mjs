...
function summarize(transactions) {
  ...
  for (const transaction of all) {
    accountBreakdown[transaction.accountId] = (accountBreakdown[transaction.accountId] ?? 0) + 1;
    accountAmountBreakdown[transaction.accountId] =
      (accountAmountBreakdown[transaction.accountId] ?? 0) + transaction.correctedAmount;
  }
  for (const transaction of topLevel) {
    ...
    topLevelAccountAmountBreakdown[transaction.accountId] =
      (topLevelAccountAmountBreakdown[transaction.accountId] ?? 0) + transaction.correctedAmount;
  }
  const dates = all
    .map((transaction) => transaction.correctedTransactionDate)
    .filter(Boolean)
    .sort();
  const amountTotal = all.reduce((sum, transaction) => sum + transaction.correctedAmount, 0);
  ...
  return {
    ...
    accountBreakdown: Object.fromEntries(
      Object.entries(accountBreakdown).sort(([left], [right]) => left.localeCompare(right)),
    ),
    topLevelAccountBreakdown: Object.fromEntries(
      Object.entries(topLevelAccountBreakdown).sort(([left], [right]) => left.localeCompare(right)),
    ),
    accountAmountBreakdown: Object.fromEntries(
      Object.entries(accountAmountBreakdown)
        .sort(([left], [right]) => left.localeCompare(right))
        .map(([accountId, amount]) => [accountId, Math.round(amount * 100) / 100]),
    ),
...
try {
  ...
  const legacy = {
    ...
    exactIdEditTargets: summarizeExactIdTargets(legacyEdits, legacySnapshot),
  };

  const cache = new TransactionCache(new FileRepository(userDataPath));
  ...
  const report = {
    ...
    rebuilt,
    rebuiltExactIdEditTargets: summarizeExactIdTargets(persistedEdits, persistedSnapshot),
    persistenceRoundTrip: {
      topLevelTransactionCountDelta:
        persistedRebuilt.topLevelTransactions - inMemoryRebuilt.topLevelTransactions,
      allTransactionCountDelta: persistedRebuilt.allTransactions - inMemoryRebuilt.allTransactions,
      effectiveAmountTotalDelta:
        Math.round(
          (persistedRebuilt.allTransactionAmountTotal - inMemoryRebuilt.allTransactionAmountTotal) *
            100,
        ) / 100,
    },
    parity: {
      allTransactionCountDelta: rebuilt.allTransactions - legacy.allTransactions,
      topLevelTransactionCountDelta: rebuilt.topLevelTransactions - legacy.topLevelTransactions,
      effectiveAmountTotalDelta:
        Math.round((rebuilt.allTransactionAmountTotal - legacy.allTransactionAmountTotal) * 100) /
        100,
      topLevelAmountTotalDelta:
        Math.round((rebuilt.topLevelAmountTotal - legacy.topLevelAmountTotal) * 100) / 100,
