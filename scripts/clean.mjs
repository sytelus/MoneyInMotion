...
const packageJson = JSON.parse(fs.readFileSync(path.join(repositoryRoot, 'package.json'), 'utf-8'));
...
const generatedPaths = ['core', 'server', 'web'].flatMap((packageName) => [
  ...
  path.join(repositoryRoot, 'packages', packageName, 'tsconfig.tsbuildinfo'),
]);
